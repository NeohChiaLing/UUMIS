...
@Component({
  ...
  styleUrl: './teacher-assignment.css' // Create empty css file if needed
})
export class TeacherAssignmentComponent implements OnInit, OnDestroy {

  viewState: string = 'years';
  selectedYear: string = '';
  selectedSubject: any = null;

  // --- NEW: Locked Teacher Constraints ---
  teacherAllowedYears: string[] = [];
  teacherAllowedSubjects: string[] = [];
  years: string[] = []; // Will only be populated with teacher's assigned years!
  ...
  // --- NEW: Fetch Teacher's Locked Profile ---
  loadTeacherProfile() {
    ...
    this.authService.getUsers().subscribe({
      next: (users: any[]) => {
        ...
        if (myFreshProfile) {
          ...
          this.teacherAllowedYears = classList.map((c: string) => {
          ...
          // Assign strictly to the UI loop
          this.years = this.teacherAllowedYears;
        }
      }
    });
  }
  ...
  // --- FIX: Filter subjects by Year AND Teacher's Assigned Subjects! ---
  selectYear(year: string) {
    ...
    const filteredSubjects = this.allSubjectsFromDb.filter(s =>
      s.yearGroup === year && this.teacherAllowedSubjects.includes(s.name)
    );
